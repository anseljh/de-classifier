"""
The nxt whack at classifying docket entries!
Uses output3.csv, which is labeled data created with labeler3.py.
...
def load_dataset():
    ...
    for row in rows:
        description, label, docket_id, de_id, doc_id = row
        if de_id not in dataset:  # ignore duplicate rows
            dataset[de_id] = {
